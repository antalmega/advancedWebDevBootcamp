...
async function hasMostFollowers() {
    ...
    let dataReq = Array.from(arguments).map(user => $.getJSON(baseUrl + user));
    let dataResult = await Promise.all(dataReq);
    let max = dataResult.sort((a,b) => b.followers - a.followers)[0];
...
async function starWarsString(id) {
    ...
    let people = await $.getJSON(url)
    let film = await $.getJSON(people.films[0]);
    let planet = await $.getJSON(film.planets[0]);
    return `${people.name} is featured in ${film.title}, directed by ${film.director} and it takes place on ${planet.name}`
}